...
const FloatingElements: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const whatIsItSection = document.getElementById('what-is-it-section');
      const pricingSection = document.getElementById('pricing-section');

      if (whatIsItSection) {
        const rect = whatIsItSection.getBoundingClientRect();
        // Aparece cuando el usuario está terminando la sección (el fondo de la sección entra en pantalla)
        setShowCTA(rect.bottom < window.innerHeight);
      }
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-6 right-6 z-[60] transition-all duration-700 transform ${showCTA ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-24 opacity-0 scale-90 pointer-events-none'
          }`}
      >
        <button
      ...
      <div
        className={`fixed bottom-6 left-6 z-[60] transition-all duration-700 transform ${showWhatsApp && isWppVisible ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-24 opacity-0 scale-90 pointer-events-none'
          }`}
      >
